...
import React, { useCallback, useEffect, useState } from 'react'
 ...
 const  Card =() => {  
  ...
  const [jsonCardElements, changeJsonCardElements] = useState([]);
 

  useEffect( ()=>  { 
    axios.get('https://64257abd9e0a30d92b333901.mockapi.io/card').then((rsp)=> {
      changeJsonCardElements(rsp.data);
    });
  
  },  []);
  let  allPrice = 0; 
  const cardElements  = jsonCardElements.map((element, id )=> {
    allPrice+=element.pricePizza;
 
    return (
      <div className="cart__item">
      <div className="cart__item-img">
        <img className="pizza-block__image" src={element.imageUrlPizza} alt="Pizza" />
      </div>
      <div className="cart__item-info">
        <h3>{element.tittlePizza}</h3>
        <p>{element.propertyPizza}.</p>
      </div>
      ...
      return (
          ...
          <div className="cart__top">
            <h2 className="content__title"><svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              Корзина</h2>
            <div className="cart__clear" onClick={()=>{changeJsonCardElements([])}}>
        ...
        {cardElements} 
          ...
          <div className="cart__bottom">
            <div className="cart__bottom-details">
              <span> Всего пицц: <b>{jsonCardElements.length}</b> </span>
              <span> Сумма заказа: <b>{allPrice} ₽</b> </span>
            </div>
